...
export class WalletComponent implements OnInit {
  ...
  handleRangeValue(value) {
    ...
    this.value = value
    const dataGetListFashion = new HttpRequestModel();
    dataGetListFashion.params = { value: value, type: 5, sort: this.sort };
  ...
  handleValue(e) {
    ...
    const dataGetListFashion = new HttpRequestModel();
    dataGetListFashion.params = { sort: e.value, type: 5, value: this.value };
    this.getListFashion.getListFashion(dataGetListFashion).subscribe((res) => {
      this.spinner.hide()
      this.totalPage = res.countPage
      this.dataProduct = res.data.map((data) => new CardOneModel(data));
  ...
  handlePageChange(e) {
    ...
    this.currentPage = e;
    const dataGetListFashion = new HttpRequestModel();
    dataGetListFashion.params = { currentPage: this.currentPage, type: 5 };
    this.getListFashion.getListFashion(dataGetListFashion).subscribe((res) => {
      this.spinner.hide()
      this.dataProduct = res.data.map((data) => new CardOneModel(data));
    }, (error) => {
